...
"""Thread and ThreadGroup that reraise exceptions on the kevin.main thread."""
...
class ReraiserThreadGroup(object):
  ...
  def _JoinAll(self, watcher=None, timeout=None):
    ...
    immediately on exceptions raised on the kevin.main thread.
    ...
    try:
      while alive_threads and (end_time is None or end_time > time.time()):
        for thread in alive_threads[:]:
          ...
          # Allow the kevin.main thread to periodically check for interrupts.
          thread.join(0.1)
  ...
  def JoinAll(self, watcher=None, timeout=None,
    ...
    Reraises exceptions raised by the child threads and supports breaking
    immediately on exceptions raised on the kevin.main thread. Unfinished threads'
